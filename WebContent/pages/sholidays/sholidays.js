(function(jq){
	...
	function dayInfo(is67,ymd,j){
		var ht="";
		if(is67){
			ht="<div class='dayH' date='"+ymd+"' title='点击切换工作日和休息日'><span class='stateday'>休息日</span>"+"<span class='spanday'>"+(j<10?("0"+j):j)+"</span>"+"</div><div class='alltitle'></div>";
		}else{
			ht="<div class='dayH' date='"+ymd+"' title='点击切换工作日和休息日'><span class='stateday'>工作日</span>"+"<span class='spanday'>"+(j<10?("0"+j):j)+"</span>"+"</div><div class='alltitle'></div>";
		}
		return ht;
	}
	/**
		...
		for (i = 1; i <= CirNum; i++){
	    	...
	    	if (i < FirstDate + 1 || i > ErtNum){//非工作日
				...
				var is67=(i % 7 == 0||(i+6)%7==0)?"is67 ":"";//周六或者是周qi
		      	Temp += (SY == Y && SM == M && SD == j ? "<td is67='"+(is67?2:1)+"' class='"+is67+"now' id='"+ymd+"'>" : "<td is67='"+(is67?2:1)+"' class='"+is67+"nonow' id='"+ymd+"'>") + dayInfo(is67,ymd,j) + "</td>";
		   		j = j + 1;
			}
			if (i % 7 == 0 && i < ErtNum){
				Temp += "</tr><tr>";
