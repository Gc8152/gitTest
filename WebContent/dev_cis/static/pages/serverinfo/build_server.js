...
$(function(){
	getServerInfo();
	initVlidate($("#serverInfoForm"));
	
	function getServerInfo(){
		baseAjaxJsonp(contextpath + "server_info/list/01", null, function(result){
			//下拉菜单select2
			$("select").select2();
			if(result.total==1){
				...
				$("[name='serverinfoAdditional.username']").val(serverInfo["serverinfoAdditional"]["username"]);
				$("[name='serverinfoAdditional.password']").val(serverInfo["serverinfoAdditional"]["password"]);
				$("[name='serverinfoAdditional.port']").val(serverInfo["serverinfoAdditional"]["port"]);
			}
