;function initconfigManageHandleLayout(id, config_id){
	...
	submit.click(function(e){
		...
		baseAjaxJsonp(dev_project+"NotconformHandle/notconformHandleHandle.asp?call="+call+"&SID="+SID+"&"+content, null, function(data){
	...
	function initLayout(){
		...
		initSelect(currTab.find("#config_status"),{value:"ITEM_CODE",text:"ITEM_NAME"},{dic_code:"P_DIC_CONFIG_NOCONFORM_STATUS"},null,null,arr);
		var content = form.serialize();
		var call = getMillisecond();
		baseAjaxJsonp(dev_project+"NotconformRaise/notconformRaiseFindRecordById.asp?call="+call+"&SID="+SID+"&"+content, null, function(result){
			...
			if((result["STATUS"]=="05")||(result["STATUS"]=="06")){
				initSelect(currTab.find("#config_status"),{value:"ITEM_CODE",text:"ITEM_NAME"},{dic_code:"P_DIC_CONFIG_NOCONFORM_STATUS"},result["STATUS"],null,arr);
			}
			initTable();
		},call);
	}
