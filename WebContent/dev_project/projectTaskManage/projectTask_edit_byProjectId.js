...
function codeTask(seles){
	...
	var PROJECT_TYPE_NAME=seles.PROJECT_TYPE_NAME;
	...
	function initRefresh(){
		...
		var arr = {};
		arr.ID=project_id;
		arr.task_id=seles.task_id;
		arr.tId="treeTestTask_1";
		arr.PID="";
		initTask("treeTestTask",arr);
	...
	function initTask(treeId,treeNode){
		...
		$page.find("button[name='func_add']").unbind("click").click(function(){
			...
			add_task_Pop({ SYSTEM_ID: system_id,SYSTEM_NAME: system_name,TYPE:"add",
									PROJECT_ID:project_id,TASK_NUM: treeNode.ID});
		});
		...
		//getCurrentPageObj().find("button[name='func_upload']").unbind("click").click(function(){
			importExcel.initImportExcel($page.find("button[name='func_upload']"),"开发任务信息","sfile/downloadFTPFile.asp?id=m_052","projectTask/importTest.asp?PROJECT_ID="+project_id,function(msg){
	...
	$page.find("button[name='func_delete_btn']").unbind("click").click(function(){
		...
		if(rows.length>0){
			...
			ids = ids.substring(1);
			deleteFunc(rows[0].PROJECT_ID,ids,rows);
		}		
	});
...
//开发任务刷新
function refreshFuncTable(project_id){
	getCurrentPageObj().find("[tb='funcSplitTable']").bootstrapTable('refresh',{
	url:dev_project+"projectTaskManager/queryQeqByProjectId.asp?SID=" + SID + "&call=" + fsCall +"&PROJECT_ID="+project_id});
	
}
//删除测试功能点
function deleteFunc(project_id,task_id,rows){
