...
//初始化操作日志
function initqHInfoOperateLogList(risk_id){
	...
	getCurrentPageObj().find("#qHInfoOperateLogTable").bootstrapTable("destroy").bootstrapTable({
		//请求后台的URL（*）
...
function initqHInfoHandelLogList(risk_id){
	var queryParams=function(params){
		var temp={
				limit: params.limit, //页面大小
				offset: params.offset //页码
	...
	getCurrentPageObj().find("#qHInfoHandleLogTable").bootstrapTable("destroy").bootstrapTable({
		//请求后台的URL（*）
...
/****************************初始化页面信息**********************************/	
//初始化申请信息
function initqHQueryInfo(item){
	for(var k in item){
		k1 = k.toLowerCase();
		 if((k1=="risk_grade")&&(item[k]!="01")){
	    	   getCurrentPageObj().find("#qHq_info_project_tr").hide();
	       }
       getCurrentPageObj().find("[name='QH."+ k1 +"']").text(item[k]);
	}
