function initSendProduceVerifyEditLayout(item){
	var currTab = getCurrentPageObj();
	
	//初始化需求点基本信息
	initSubReqInfo(item);
	function initSubReqInfo(item){
		var $inputs = currTab.find("input");
	...
	function getParamFromCurrTab(){
		...
		param["VERIFY_RESULT"] = currTab.find("#VERIFY_RESULT").val();
		param["VERIFY_DESC"] = currTab.find("#VERIFY_DESC").val();
		return param;
	}
	
	function saveOrCommitVerifyResult(param){
		var call = getMillisecond();
		baseAjaxJsonp(dev_construction+'sendProduceVerify/saveVerifyResult.asp?call='+call+'&SID='+SID, param, function(result){
			if(result.result=="true"){
				//保存信息
				alert("保存验证信息成功", function(){
					closeCurrPageTab();
				});
			} else {
				alert("保存验证信息失败");
	...
	//初始化按钮事件
	currTab.find("#save").unbind("click").click(function(){
		var param = getParamFromCurrTab();
		param["IS_COMMIT"] = "01";
		saveOrCommitVerifyResult(param);
	});
	
	currTab.find("#commit").unbind("click").click(function(){
		var param = getParamFromCurrTab();
		param["IS_COMMIT"] = "00";
		saveOrCommitVerifyResult(param);
	});
