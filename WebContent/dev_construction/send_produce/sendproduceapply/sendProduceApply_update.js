 ...
 * @param p
 */
function initSendProInfoUpdate(p) {
	//投产单基本信息
	...
	function refreshTaskTable(system_id, versions_id){
		...
		getCurrentPageObj().find("#sendProContent").bootstrapTable({
			url : dev_construction+'sendProduceApply/queryTaBySysAndVer.asp?call='+call_sendTask+'&SID='+SID+'&system_id='+system_id+'&version_id='+versions_id,
			//url : dev_construction+'sendProduceApply/querySendTaskByAuditNo.asp?call='+call_sendTask+'&SID='+SID+'&audit_no='+p,
			//url : dev_construction+'sendProduceApply/queryTaskBySysAndVer.asp?call='+call_sendTask+'&SID='+SID+'&system_id='+system_id+'&versions_id='+versions_id,
			//url : dev_construction+'sendProduceApply/queryTaskForUpdate.asp?call='+call_sendTask+'&SID='+SID+'&system_id='+system_id+'&versions_id='+versions_id,
			method : 'get', //请求方式（*）   
			...
			onLoadSuccess:function(data){
				gaveInfo();
				originTaskId = $.map(data.rows, function(item, index) {
					return item.REQ_TASK_ID;
				});
				var currTab = getCurrentPageObj();
				currTab.find(".file_template_style").hide();
				var sendData = data.rows;
			...
			columns : [ {
				...
				formatter: function (value, row, index) {
					...
					'onclick="viewTaskInfo('+row.REQ_TASK_ID+')">'+value+'</span>';
				}
			},{
				width : "130",
				field : 'REQ_TASK_NAME',
				...
				align : "center",
				formatter:function(value, row, index) {
					var edit='<a class="click_text_sp" style="margin-right: 10px" onclick="detailSendProTaskUpdate(\''+index+'\');">查看</a>'
					if(row.IS_PEEL=="00"){
	...
	addfile.click(function(){
		...
		var sys_name = getCurrentPageObj().find("#system_name").val().toUpperCase();
		var ver_name = getCurrentPageObj().find("#versions_name").val();
		if(sys_name==""||ver_name==""){
			alert("请先选择应用及版本");
			return ;
	...
	getCurrentPageObj().find("#submit").click(function() {
		...
		if(result.result == true){
			...
			nconfirm("确定要提交审计吗？",function(){
					var call = getMillisecond();
					var url = dev_construction+'sendProduceApply/saveSendProInfo.asp?call='+call+'&SID='+SID+'&submit=2';
					result.param["approve_status"] = "01";//投产单待提交投产
					baseAjaxJsonp(url, result.param, function(data){
						if (data != undefined&&data!=null&&data.result=="true") {
							alert("提交成功!");
							closeCurrPageTab();
		...
		if(fileData!=null&& fileData.length>0){
			for(var i=0; i<fileData.length; i++){
				if(!fileCheck){
					...
					fileCheck = (fileType.indexOf("sj_") >= 0);
				}
			}
		} else {
