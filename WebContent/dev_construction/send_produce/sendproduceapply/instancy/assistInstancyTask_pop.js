//页面div调用pop的id，请求参数
function openNoSendProduceTaskPop(id,param){
	getCurrentPageObj().find('#myModal_assisttask').remove();
	getCurrentPageObj().find("#"+id).load("dev_construction/send_produce/sendproduceapply/assistTask_Pop.html",{},function(){
		getCurrentPageObj().find("#myModal_assisttask").modal("show");
...
function initAssistTaskTable(id,param){
	...
	var queryAssistCall = getMillisecond()+"2";
	getCurrentPageObj().find('#assisttask_table').bootstrapTable("destroy").bootstrapTable({
				url :dev_construction+'sendProduceApply/querySendInstancyTaskByTaskIdForCheck.asp?call='+queryAssistCall+'&SID='+SID
				+'&req_task_ids='+param.req_task_ids+'&plan_date='+param.plan_date,
				method : 'get', // 请求方式（*）
