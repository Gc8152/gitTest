//查看投产条件
function openCheckTable(pop_id,req_task_code){
	getCurrentPageObj().find('#check_div').remove();	
	getCurrentPageObj().find("#"+pop_id).load("dev_construction/send_produce/sendproduceapply/instancy/check_table.html",{},function(){
		initCheckTable(req_task_code);
	});
}

function initCheckTable(req_task_code){
	getCurrentPageObj().find("#sendProInContentDetail2").hide();
	getCurrentPageObj().find("#sendProConfirmContentCon").hide();//隐藏紧急需求的任务检验table
	var produce_standard = new Object();
	//紧急版
	produce_standard["15"] = [true, true, true, true, false, false, false, false, false, false, true, true, false, false];
	//1618
	produce_standard["other"] = [true, true, true, true, true, true, true, true, true, true, true, true, true, true];

	var detailTable = getCurrentPageObj().find("#sendProDetailCheckTable");
