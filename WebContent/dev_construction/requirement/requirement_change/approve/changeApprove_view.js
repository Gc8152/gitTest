...
function changeApproveView(selRow){//初始化
	...
	function initFileTable(selRow){
		...
		//附件列表
		 var tablefile = $page.find("#approvev_fileTable");
		 var business_code = selRow.FILE_ID;
		 getSvnFileList(tablefile, $page.find("#file_approvev_modal"), business_code, "00");
	...
	/*初始化需求点列表*/
	var approveviewCall = getMillisecond();//需求点表回调方法
	var sUrl = dev_construction+"requirement_change/queryReqSubs.asp?SID=" + 
			SID + "&call=" + approveviewCall + "&REQ_ID=" + selRow.REQ_ID;
	$page.find("#approveview_reqSubsTable").bootstrapTable({
		url :sUrl,
		method : 'get', // 请求方式（*）
