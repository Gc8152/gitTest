...
function changeQueryView(selRow){//初始化
	...
	/*初始化需求点列表*/
	var queryCall = getMillisecond();//需求点表回调方法
	var sUrl = dev_construction+"requirement_change/queryReqSubs.asp?SID=" + 
			SID + "&call=" + queryCall + "&REQ_ID=" + selRow.REQ_ID;
	$page.find("#query_reqSubsTable").bootstrapTable({
		url :sUrl,
		method : 'get', // 请求方式（*）
...
//		}
	...
	function initFileTable(selRow) {
		...
		//附件列表
		 var tablefile = $page.find("#query_fileTable");
		 var business_code = selRow.FILE_ID;
		 getSvnFileList(tablefile, $page.find("#file_query_modal"), business_code, "00");
