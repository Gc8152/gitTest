initTaskAnalyListBtnp();
initFollowerTaskQueryp();
initTaskAnalyDicCodep();

//加载任务状态字典
function initTaskAnalyDicCodep(){
...
//初始化按钮
function initTaskAnalyListBtnp() {

	// 查询
	getCurrentPageObj().find("#serach_P").click(function() {
		...
		var task_state=$.trim(curTab.find("#task_state").val());
		var req_task_relation = $.trim(curTab.find('#req_task_relation').val());
		var req_task_type = $.trim(curTab.find('#req_task_type').val());
		...
		getCurrentPageObj().find('#pGFollowerTaskTable').bootstrapTable('refresh',{
			...
			+'&req_task_relation='+req_task_relation+'&req_task_type='+req_task_type+'&personal=personal'+'&task_state='+task_state
		});
	});
	enterEventRegister(getCurrentPageObj().attr("class"), function(){getCurrentPageObj().find("#serach_P").click();});

	//重置
	getCurrentPageObj().find('#reset_P').click(function() {
		  getCurrentPageObj().find("#PfollowerQuery input").val("");
			var selects = getCurrentPageObj().find("#PfollowerQuery select");
			selects.val(" ");
			selects.select2();
	});
...
//初始化列表
function initFollowerTaskQueryp() {
	...
	getCurrentPageObj().find('#pGFollowerTaskTable').bootstrapTable("destroy").bootstrapTable({
		url :dev_construction+"GFollowerTask/queryFollowerTaskList.asp?SID="+SID+'&personal=personal',
		method : 'get', // 请求方式（*）
		...
		columns : [{
			...
			formatter : function(value, row, index) {
				return req_namformatter(32,value,"color: blue;",' onclick="openReqTaskDetailp(\''+row.REQ_TASK_ID+'\')";');
			}
		}, {
			field : "SYSTEM_NO",
			title : "应用ID",
			...
			align : "center",
			width : 110,
			formatter:function(value,row,index){return '<a style="color:blue" href="javascript:void(0)" onclick="openSystemDetailp(\''+row.SYSTEM_NO+'\')";>'+value+'</a>';}
			...
			formatter:function(value,row,index){
				if(row.TASK_TYPE=='05'){
					return '<a style="color:blue" href="javascript:void(0)" onclick="openEmSubReqDetailp(\''+row.REQ_ID+'\')";>'+value+'</a>';
				}else{
					return '<a style="color:blue" href="javascript:void(0)" onclick="openSubReqDetailp(\''+row.REQ_ID+'\')";>'+value+'</a>';
				}
			}
		}, {
			field : "VERSION_ID",
			...
			align : "center",
			width : 180,
			formatter:function(value,row,index){if(value!=undefined&&value!=" "){return '<a style="color:blue" href="javascript:void(0)" onclick="openVersionDetailp(\''+row.VERSION_ID+'\')";>'+value+'</a>';}return '--';}
			...
			align : "center",
			width : 80,
			/*formatter:function(value,row,index){return '<a style="color:blue" href="javascript:void(0)" onclick="openReqTaskDetailp(\''+row.REQ_TASK_ID+'\')";>'+value+'</a>';}*/
			...
			align : "center",
			width : 160,
			formatter:function(value,row,index){return '<a style="color:blue" href="javascript:void(0)" onclick="openReqDetailp(\''+row.REQ_ID+'\')";>'+value+'</a>';}
		}, {
			field : "RELEASE_NUM",
			title : "关联任务数",
			align : "center",
...
//						formatter:function(value,row,index){if(value>0){return "查看";}return "--";}
			formatter:function(value,row,index){if(value>0){return '<a style="color:blue" href="javascript:void(0)" onclick="phasedFollowerp(03,\''+row.REQ_TASK_ID+'\',\''+row.REQ_TASK_CODE+'\')";>查看</a>';}return "--";}
		},{
			field : "SUMMARY_STATE",
			title : "设计开发",
			align : "center",
			width : 80,
			formatter:function(value,row,index){if(value>0){return '<a style="color:blue" href="javascript:void(0)" onclick="phasedFollowerp(05,\''+row.REQ_TASK_ID+'\',\''+row.REQ_TASK_CODE+'\')";>查看</a>';}return "--";}
		},{
			field : "DETAILDESIGN_STATE",
			title : "详细设计",
			align : "center",
			formatter:function(value,row,index){if(value>0){return '<a style="color:blue" href="javascript:void(0)" onclick="phasedFollowerp(06,\''+row.REQ_TASK_ID+'\',\''+row.REQ_TASK_CODE+'\')";>查看</a>';}return "--";},
			visible:false,
		},{
			...
			align : "center",
			width : 100,
			formatter:function(value,row,index){if(value>0){return '<a style="color:blue" href="javascript:void(0)" onclick="phasedFollowerp(09001,\''+row.REQ_TASK_ID+'\',\''+row.REQ_TASK_CODE+'\')";>查看</a>';}return "--";}
		},{
			field : "UAT_STATE",
			title : "UAT测试",
			align : "center",
			formatter:function(value,row,index){if(value>0){return '<a style="color:blue" href="javascript:void(0)" onclick="phasedFollowerp(10,\''+row.REQ_TASK_ID+'\',\''+row.REQ_TASK_CODE+'\')";>查看</a>';}return "--";},
			visible:false,
		},{
			field : "OLD_TASK_ID",
			title : "存量任务编号",
...
//打开任务详情页面
function openReqTaskDetailp(req_task_id){
...
//打开应用详情页面
function openSystemDetailp(ids){
...
//打开需求点详情页面
function openSubReqDetailp(req_id){
...
//打开紧急需求点详情页面
function openEmSubReqDetailp(req_id){
...
//打开版本详情页面
function openVersionDetailp(version_id){
...
//打开需求360详情页面
function openReqDetailp(req_id){
...
//根据不同阶段的任务信息
function phasedFollowerp(task_state,req_task_id,req_task_code){
	var text="";
