...
function interUseApply(){
	var req_task_state=getCurrentPageObj().find("#TDreq_task_state_id").val();
	if(req_task_state=="12"||req_task_state=="13"||req_task_state=="14"||req_task_state=="15"){
		alert("该任务无法申请接口！");
		return;
	}
	var myArray=new Array()
	myArray["req_task_id"]=getCurrentPageObj().find("#TDreq_task_id").val();
	myArray["req_task_code"]=getCurrentPageObj().find("#TDreq_task_code").text();
	myArray["con_system_id"]=getCurrentPageObj().find("#TDsystem_no").val();
	myArray["req_task_name"]=getCurrentPageObj().find("#TDreq_task_name").text();
	myArray["con_system_name"]=getCurrentPageObj().find("#TDsystem_name").text();
	 ...
	 closeAndOpenInnerPageTab("ApplyAddOrder_appList","填写申请单","dev_application/useInterfaceApply/useInterfaceApply_edit.html", function(){
		 initUseAppOrderAddOrUpdateLayOut(myArray,"edit");
		});
 }
...
function viewDetailPop(REQ_TASK_ID){
	...
	//查询里程碑阶段（任务字典）
	baseAjaxJsonp(dev_construction+"GFollowerTask/queryTaskInfomation.asp?SID="+SID+'&call='+mileCall+'&req_task_id='+REQ_TASK_ID, null, function(data) {
    	if (data != undefined && data != null && data.result=="true") {
    		var stCall = getMillisecond();
    		var item = data.rows;
			// 流信息查询
			baseAjaxJsonp(dev_resource+'StreamApply/queryStreamApply.asp?SID='+SID+'&call='+stCall+'&req_task_id='+item.REQ_TASK_ID+'&limit=5'+'&offset=0'+'&task_info='+'00', null, function(data) {
		    	if (data != undefined && data != null && data.result=="true") {		
			    	var row = data.rows;
			    	if(row.length != 0){
	    	...
	    	//遍历阶段
	    	var stream_name=getCurrentPageObj().find('#TDsystem_name').val();
	    	for(var k in phase){
	    		var p;
...
function needToCheck(status,rows,is_pmo,stream_name){
	...
	if(status ==  '05'){
		var analysisCall = getMillisecond();
		//设计开发查询
		baseAjaxJsonp(dev_construction+"GTaskPhased/queryOneRecord.asp?SID="+SID+'&call='+analysisCall+'&req_task_id='+rows.REQ_TASK_ID+'&type=1', null, function(data) {
	    	if (data != undefined && data != null && data.result=="true") {			
	    		var item = data.data;
	    		var phased_state = item["PHASED_STATE"]; //文档上传
	    		var jury_phased = item["JURY_PHASED"]; //评审结论
				//文档上传
				if(phased_state==" " || phased_state==undefined || phased_state==null){
					var cp = getCurrentPageObj().find("#d_analysis_doc");
					cp.removeClass("hide");
					cp.addClass("tag-blue");
					cp.unbind("click");
					cp.bind("click", function(){
				...
				//一级评审(应用中测试经理),(文档上传成功后，一级未审批，一级审批不通过)
				/*else if(jury_phased==" " || jury_phased==undefined || jury_phased==null || jury_phased =='03'){
					var cp = getCurrentPageObj().find("#d_analysis_exa_first");
					cp.removeClass("hide");
					cp.addClass("tag-blue");
					cp.unbind("click");
					//页面跳转
					cp.bind("click", function(){
						var records=null;
						baseAjaxJsonp(dev_construction+"GTaskPhased/queryTaskByReqId.asp?SID="+SID+"&req_task_id="+rows.REQ_TASK_ID+"&phased_state=03", null , function(data) {
							if (data != undefined && data != null && data.result=="true") {
								 records=data.rows;
								 closeAndOpenInnerPageTab("edit_jury","发起设计开发一级评审","dev_construction/jury/conductPR/juryInfo/preparePR.html",function(){
									getCurrentPageObj().find('#req_task_state').val('03');
				...
				//二级评审(1618,PMO),(因项目经理不会是PMO所以不会跳转)
				/*else if(jury_phased=="02" && rows["IS_1618"] == '00'){
					var cp = getCurrentPageObj().find("#d_analysis_exa_second");
					cp.removeClass("hide");
					if(is_pmo == "1"){
						cp.addClass("tag-blue");
						cp.unbind("click");
						cp.bind("click", function(){
							closeAndOpenInnerPageTab("d_analysis_exa_second","设计开发评审","dev_construction/requirement/reqTask_phased/analyze/task_analyze_querylist.html");
						});
					}
				}*/
	    	}else{
	...
	//单元测试
		...
		var unitCall = getMillisecond();
		//单元测试查询
		baseAjaxJsonp(dev_construction+"GTaskPhased/queryOneRecord.asp?SID="+SID+'&call='+unitCall+'&req_task_id='+rows.REQ_TASK_ID+'&type=2', null, function(data) {
	    	if (data != undefined && data != null && data.result=="true") {			
	    		var item = data.data;
	    		var phased_state = item["PHASED_STATE"];
	    		if(phased_state==" " || phased_state==undefined || phased_state==null){
	    			//文档上传
	    			var cp = getCurrentPageObj().find("#d_unitTest_doc");
	    			cp.removeClass("hide");
	    			cp.addClass("tag-blue");
	    			cp.unbind("click");
	    			cp.bind("click", function(){
	...
	//联调测试
		...
		var debuggingCall = getMillisecond();
		//联调测试查询
		baseAjaxJsonp(dev_construction+"GTaskPhased/queryOneRecord.asp?SID="+SID+'&call='+debuggingCall+'&req_task_id='+rows.REQ_TASK_ID+'&type=3', null, function(data) {
	    	if (data != undefined && data != null && data.result=="true") {			
	    		var item = data.data;
	    		var phased_state = item["PHASED_STATE"];
	    		if(phased_state==" " || phased_state==undefined || phased_state==null){	
	    			//文档上传
	    			var cp = getCurrentPageObj().find("#d_debugging_doc");
	    			cp.removeClass("hide");
	    			cp.addClass("tag-blue");
	    			cp.unbind("click");
	    			cp.bind("click", function(){
	...
	//SIT测试（需要先申请流）
	else if(status == '09' && (stream_name != null || stream_name != ' ' || stream_name != undefined)) {
		var sitCall = getMillisecond();
		//SIT测试查询
		baseAjaxJsonp(dev_construction+"GTaskPhased/queryOneRecord.asp?SID="+SID+'&call='+sitCall+'&req_task_id='+rows.REQ_TASK_ID+'&type=4', null, function(data) {
	    	if (data != undefined && data != null && data.result=="true") {	
				...
				if(status==" " || status==undefined || status==null || status == '02' || status == '05'){
					...
					cp.unbind("click");
					cp.bind("click", function(){
						var version_type = getCurrentPageObj().find("#TDversion_type").val();
						if(version_type=='15'){
	...
	//UAT测试（需要先申请流）
	else if(status == '10' && (stream_name != null || stream_name != ' ' || stream_name != undefined)){
		var uatCall = getMillisecond()+'1';
		//UAT测试查询
		baseAjaxJsonp(dev_construction+'UatReport/queryAllReqSubReport.asp?call='+uatCall+'&SID='+SID+'&req_task_id='+rows.REQ_TASK_ID, null, function(data) {
	    	if (data != undefined && data != null) {
	    		var task_state=rows.REQ_TASK_STATE;
	    		if(parseInt(task_state)<13||parseInt(task_state)==90){//任务未完成才需要提示
	    		  var item = data.rows[0];
	    		  var upload_state = item["UPLOAD_STATE"];
	    		  var accept_result = item["ACCEPT_RESULT"];
	    		  var person_name = item["CREATE_PERSON_NAME"];
	    		  if(upload_state != '00' && accept_result != '00'){
	    			var cp = getCurrentPageObj().find("#d_uatTest_doc");
	    			cp.removeClass("hide");
	    			getCurrentPageObj().find("#s_uatTest_doc").text(":"+person_name);
	    			//文档上传(需求提出人)
	    			if(item.CREATE_PERSON == person_id){
	    				getCurrentPageObj().find("#s_uatTest_doc").remove();
	    				cp.addClass("tag-blue");
	    				cp.unbind("click");
	    				cp.bind("click", function(){
	    					closeAndOpenInnerPageTab("uatReport_edit","UAT报告上传页面","dev_construction/uat_test/uatreport/uatReport_edit.html",function(){
	    						initUatReportInfo(item);
	    					});
	    				});
	    			 }
	    		  }
	...
	//提交投产（需要先申请流）
	else if(status == '12' && (stream_name != null || stream_name != ' ' || stream_name != undefined)){
		var sendProductCall = getMillisecond();
		//提交投产查询
		baseAjaxJsonp(dev_construction+'GFollowerTask/querySendProductMsg.asp?call='+sendProductCall+'&SID='+SID+'&system_id='+rows.SYSTEM_NO+'&versions_id='+rows.VERSION_ID+'&req_task_id='+rows.REQ_TASK_ID, null, function(data) {
			if (data != undefined && data != null && data.result=="true") {
				var row = data.row;
