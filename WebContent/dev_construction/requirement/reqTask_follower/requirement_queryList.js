 ...
 * @returns {String}
 */
function RequirementQueryUrl(){
	var url = dev_construction+"GFollowerTask/queryAllReqInfo.asp?SID="+SID;
	var fds=getCurrentPageObj().find("#reqTerm [name]");
 ...
 * @returns 
 */
function getReqQueryParam(){
	var param={};
	var finds=	getCurrentPageObj().find("#reqTerm [name]");
...
//获取时间
var requirement_queryList_call = getMillisecond();
/**
 * 查询列表显示table
 */
(function () {
	var queryParams=function(params){
		...
		temp["call"] = requirement_queryList_call;
		return temp;
	};
	getCurrentPageObj().find("#reqTable").bootstrapTable(
			{
				url : dev_construction+'GFollowerTask/queryAllReqInfo.asp?SID='+SID,
				method : 'get', //请求方式（*）   
				...
				singleSelect: true,
				jsonpCallback: requirement_queryList_call,
...
(function(){
	...
	getCurrentPageObj().find("#reset").click(function(){
		getCurrentPageObj().find("#reqTerm input").val("");
		var selects=$("#reqTerm select");
		selects.val(" ");
		selects.select2();
	});
	...
	getCurrentPageObj().find("#query").click(function(){
		getCurrentPageObj().find("#reqTable").bootstrapTable("refresh",{url:RequirementQueryUrl()});
	});
	enterEventRegister(getCurrentPageObj().attr("class"), function(){getCurrentPageObj().find("#query").click();});
	//360视图按钮
	getCurrentPageObj().find("#view").unbind("click");
	getCurrentPageObj().find("#view").click(function(){
		var selects = getCurrentPageObj().find("#reqTable").bootstrapTable("getSelections");
		if(selects.length != 1) {
			alert("请选择一条数据进行操作！");
			return;
		}
