...
function initTaskAnalyListBtn() {
	...
	getCurrentPageObj().find("#serach_myTask_Query").click(function() {
		...
		getCurrentPageObj().find('#g_myTaskTable').bootstrapTable('refresh',{
			url:dev_construction+"GFollowerTask/queryTaskList.asp?SID="+SID
	...
	getCurrentPageObj().find('#tosee_taskDetail').click(function() {
		...
		var req_type=$.map(id, function (row) {return row.REQ_ACC_CLASSIFY;});
		var req_task_state = $.map(id, function (row) {return row.REQ_TASK_STATE;});
		/*if(req_task_state == '00' || req_task_state == '01' || req_task_state == '02' || req_task_state == '03' || req_task_state == '14' || req_task_state == '15'){
			alert('该任务状态不能查看待办');
			return;
		}*/
		if(req_type=='00'){
			if(id.length==1){
				...
				closeAndOpenInnerPageTab("req_taskDet","任务详情页面","dev_construction/requirement/reqTask_follower/myTask_urgentinfo.html",function(){
				 ...
				 if (data != undefined && data != null && data.result=="true") {
				    for(var k in data){
					    ...
					    if(k=="req_code"){
							...
							getCurrentPageObj().find('#TDsub_req_code').text(str);
						}else if(k=="req_id"||k=="req_task_id"||k=="sub_req_id" ||k=="req_task_relation" || k=="version_id" || k=="version_type"){
							getCurrentPageObj().find("input[name='TD."+k+"']").val(str);
						}else if(k=="system_no"){
							getCurrentPageObj().find("#TDsystem_no").val(str);
						}else if(k=="is_cc"){
	...
	//转交
	getCurrentPageObj().find('#change_user').click(function() {
		var seles = getCurrentPageObj().find("#g_myTaskTable").bootstrapTable('getSelections');
		if(seles.length != 1){
			alert('请选择一个任务进行转交');
			return;
		}
		var state = seles[0].REQ_TASK_STATE;
		if(state == '00' || state == '01' || state == '02' || state == '12' || state == '13' || state == '14' || state == '15'){
			alert('该任务状态不允许转交');
			return;
		}
		seles[0].role = '0007';//行方项目经理
		openChangeUserPop("change_userPop",seles[0],change_user);
...
function change_user(item){
	nconfirm("确定将任务["+item.REQ_TASK_CODE+"]转交["+item.USER_NAME+"]?",function(){
		var param = {};
		param.USER_NO = item.USER_NO;
		param.REQ_TASK_ID = item.REQ_TASK_ID;
		  baseAjaxJsonp(dev_construction+"GFollowerTask/taskChangeUser.asp?SID="+SID, param , function(data) {
				 if (data != undefined && data != null && data.result=="true") {
					 alert(data.msg);
					 getCurrentPageObj().find('#g_myTaskTable').bootstrapTable('refresh',{
							url:dev_construction+"GFollowerTask/queryTaskList.asp?SID="+SID});
