...
function initTaskAnalyListBtn() {
	...
	currTab.find("#serach_TAversionQuery").click(function() {
		...
		table.bootstrapTable('refresh',{
			url:dev_construction+"GTaskPhased/queryLessTaskAnalyzeList.asp?SID="+SID+
	...
	getCurrentPageObj().find("[btn='close_modal']").click(function(){
		getCurrentPageObj().find("#audit_remarkPop6").modal('hide');
	})
...
function initTaskAnalyzeQuery() {
		...
		table.bootstrapTable("destroy").bootstrapTable({
					url :dev_construction+"GTaskPhased/queryLessTaskAnalyzeList.asp?SID="+SID+"&call="+reqAnalyzeQueryListCall,
					method : 'get', // 请求方式（*）
							 ...
							 if(value=="" || value == undefined || value==null){
								 return '待审计';
									...
									if(audit_result == '02' || audit_result == '03'){
										...
										return '<a style="color:blue" href="javascript:void(0)" onclick="showAuditRemark6(\''+remark+'\')";>'+value+'</a>';
									}else{
										return value;
									}
							 }
...
function showAuditRemark6(audit_remark){
	if(audit_remark == undefined || audit_remark == 'undefined'){ audit_remark = '';}
	getCurrentPageObj().find("#audit_remarkPop6").modal('show');
