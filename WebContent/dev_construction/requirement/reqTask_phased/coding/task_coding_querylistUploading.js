initTaskCodingListBtn();
initTaskCodingzeQuery();
initTaskCodingDicCode();

//加载任务状态字典
function initTaskCodingDicCode(){
	initSelect(getCurrentPageObj().find("#req_task_stateTO"),{value:"ITEM_CODE",text:"ITEM_NAME"},{dic_code:"G_DIC_REQTASK_STATE"});	
}

//初始化按钮
function initTaskCodingListBtn() {
	var queryForm = getCurrentPageObj().find("#reqTaskCodingForm");
	// 查询
	getCurrentPageObj().find("#serach_TversionQuery").click(function() {
		...
		getCurrentPageObj().find('#gGTaskAnalyzeTable').bootstrapTable('refresh',{
			url:dev_construction+"GTaskPhased/queryTaskCodingListUploading.asp?SID="+SID+
			"&"+param
		});
		/*
		...
		var plan_onlinetime = getCurrentPageObj().find('#plan_onlinetime_code').val();
		var plan_onlinetime1 = getCurrentPageObj().find('#plan_onlinetime_code1').val();
		getCurrentPageObj().find('#gGTaskAnalyzeTable').bootstrapTable('refresh',{
			url:dev_construction+"GTaskPhased/queryTaskCodingList.asp?SID="+SID+'&req_task_name='+escape(encodeURIComponent(req_task_name))
...
//初始化列表
function initTaskCodingzeQuery() {
