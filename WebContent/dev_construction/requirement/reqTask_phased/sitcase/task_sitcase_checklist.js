...
function initTaskSitCaseListBtn() {
	...
	getCurrentPageObj().find("#serach_TversionQuery").click(function() {
		...
		getCurrentPageObj().find('#gGTaskSitCaseTable').bootstrapTable('refresh',{
			url:dev_construction+"GTaskPhased/queryTaskSitCaseList.asp?SID="+SID+"&allUser=01"+
			"&"+param
		});
		...
		getCurrentPageObj().find('#gGTaskSitCaseTable').bootstrapTable('refresh',{
			url:dev_construction+"GTaskPhased/queryTaskSitCaseList.asp?SID="+SID+'&req_task_name='+escape(encodeURIComponent(req_task_name))
	...
	getCurrentPageObj().find("#reqSitCaseFileUP").click(function(){
		...
		if(req_task_id==null||req_task_id==undefined||req_task_id==""){
			...
			/*if(!is_same_system){
				...
				closeAndOpenInnerPageTab("task_analyze_view","测试案例文档查看","dev_construction/requirement/reqTask_phased/sitcase/task_sitcase_view.html",function(){
					var params = {};
...
function initTaskAnalyzeQuery() {
		...
		getCurrentPageObj().find('#gGTaskSitCaseTable').bootstrapTable("destroy").bootstrapTable({
					...
					columns : [ {
						...
						align : "center",
						width : 103,
						formatter:function(value,row,index){if(value=="" || value==undefined || value==null){return '<span  style="color:red; width: 110px; ";>'+"未上传"+'</span>'
;}return "已上传";}
					},{
						field : "CREATE_TIME",
						title : "创建时间",
