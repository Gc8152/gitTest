...
function initTaskSitCaseListBtn() {
	...
	getCurrentPageObj().find("#serach_TversionQuery").click(function() {
		...
		getCurrentPageObj().find('#gGTaskSitCaseTable').bootstrapTable('refresh',{
			url:dev_construction+"GTaskPhased/queryLessTaskSitCaseList.asp?SID="+SID+
			"&"+param
		});
					...
					columns : [ {
						...
						formatter:function(value,row,index){
							if(value!=undefined&&value!=" ")
								{
									return '<a style="color:blue" href="javascript:void(0)" onclick="viewSubReqDetailSitCasel(\''+row.REQ_ID+'\')";>'+value+'</a>';
								}
							return '--';
						}
					},{
...
//查看需求点详情	
function viewSubReqDetailSitCasel(req_id){
