initTaskJointListBtn();
initTaskJointzeQuery();
initTaskJointDicCode();

//加载任务状态字典
function initTaskJointDicCode(){
	initSelect(getCurrentPageObj().find("#req_task_stateTO"),{value:"ITEM_CODE",text:"ITEM_NAME"},{dic_code:"G_DIC_REQTASK_STATE"});	
}

//初始化按钮
function initTaskJointListBtn() {
	var queryForm = getCurrentPageObj().find("#reqTaskJointForm");
	// 查询
	getCurrentPageObj().find("#serach_TversionQuery").click(function() {
		var param = queryForm.serialize();//获取表单的值
		getCurrentPageObj().find('#gGTaskJointTable').bootstrapTable('refresh',{
			url:dev_construction+"GTaskPhased/queryTaskJointUnfinishedList.asp?SID="+SID+
			"&"+param
		});
		...
		var plan_onlinetime = getCurrentPageObj().find('#plan_onlinetime_joint').val();
		var plan_onlinetime1 = getCurrentPageObj().find('#plan_onlinetime_joint1').val();
		getCurrentPageObj().find('#gGTaskJointTable').bootstrapTable('refresh',{
			url:dev_construction+"GTaskPhased/queryTaskJointList.asp?SID="+SID+'&req_task_name='+escape(encodeURIComponent(req_task_name))
	...
	getCurrentPageObj().find("#reqAnalyzeFileUP").click(function(){
		var id = getCurrentPageObj().find("#gGTaskJointTable").bootstrapTable('getSelections');
		var flag = false;
...
function initTaskJointzeQuery() {
		...
		getCurrentPageObj().find('#gGTaskJointTable').bootstrapTable("destroy").bootstrapTable({
					url :dev_construction+"GTaskPhased/queryTaskJointUnfinishedList.asp?SID="+SID,
					method : 'get', // 请求方式（*）
