...
function initTaskAcceptListBtn() {
	...
	getCurrentPageObj().find("#serach_acTaskQuery").click(function() {
		...
		table.bootstrapTable('refresh',{
			url:dev_construction+"req_taskaccept/queryReqTaskList.asp?SID="+SID+
	...
	getCurrentPageObj().find("#req_taskAccept").click(function(){
		...
		var p_owner2 = $.map(id, function (row) {return row.P_OWNER2;});
		...
		var MAIN_TASK_TASTUS = $.map(id, function (row) {return row.MAIN_TASK_TASTUS;});
		//去掉主线任务为待提交投产时，协办任务无法受理的限制
		/*if(MAIN_TASK_TASTUS=="11"){
			alert("主线任务为待提交投产无法受理！");
			return;
		}*/
		if(MAIN_TASK_TASTUS=='12'||req_task_state=='13'){
		    alert("主线任务已提交投产无法受理！");
			return;
		}
		if(id.length==1){
			p_owner=p_owner[0];
			p_owner2=p_owner2[0];
			if(req_acc_classify=='00'){//紧急需求的任务
				 if(p_owner!=SID){
					  alert("您不是所选任务的当前责任人！");
					  return;
				  }
			  ...
			  if(req_task_state!="01"){
				...
				closeAndOpenInnerPageTab("req_taskAccept","任务受理页面","dev_construction/requirement/requirement_analyze/task_accept/taskAccept_update.html",function(){
					...
					initSubReqTaskList(p_owner==p_owner2);//初始化关联任务列表
					if(id[0]["REQ_ANALYSIS_NAME"]){
						getCurrentPageObj().find("#TAreq_task_analyzer").val(id[0]["REQ_ANALYSIS"]);
						getCurrentPageObj().find("#TAreq_task_analyzer_name").val(id[0]["REQ_ANALYSIS_NAME"]);
					}
...
function initTaskAcceptQueryTable() {
		...
		table.bootstrapTable("destroy").bootstrapTable({
					url :dev_construction+"req_taskaccept/queryReqTaskList.asp?SID="+SID+"&call="+taskAcceptListCall,
					method : 'get', // 请求方式（*）
