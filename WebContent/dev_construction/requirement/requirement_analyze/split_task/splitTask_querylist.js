...
function refreshSplitTaskList(param1){
	...
	table.bootstrapTable('refresh',{
		url:splitTaskQueryUrl+SID+"&call=jq_1525861721988_split&"+param+param1
	});
	isRun_split="";//
}
function initSplitTaskListLayOut(){
	...
	var queryForm = currTab.find("#splitTaskQueryForm");
	var table = currTab.find("#gSubReqInfoForSpTaskTable");	
	var spTaskListCall = "jq_1525861721988_split";
...
function initSplitTaskListBtn() {
	...
	//拆分任务确认
	currTab.find("#req_splitTaskSure").click(function(){
		...
		if(id.length==1){
			/*var sub_req_state = $.map(id, function (row) {return row.SUB_REQ_STATE;});
			 ...
			 closeAndOpenInnerPageTab("requirement_splitTask","任务拆分确认","dev_construction/requirement/requirement_analyze/split_task/splitTask_view.html",function(){
...
try{
	if(isRun_split!="false"){
		getCurrentPageObj().find("#serach_spTaskQuery").click();
	}
}catch(e){
	getCurrentPageObj().find("#serach_spTaskQuery").click();
}
...
//					url :splitTaskQueryUrl+SID+"&call="+spTaskListCall,
					method : 'get', // 请求方式（*）
