...
function initreqConvertApply(){
	...
	var table = currTab.find("#reqConvertVeiwTable");
	var form = currTab.find("#viewConvertSubmit");
	//初始化下拉选
	autoInitSelect(form);
	var tableCall = getMillisecond();
	//查询
	var query = currTab.find("#view_query");
	query.click(function(){
		...
		table.bootstrapTable('refresh',{
			url:dev_construction+'ReqConvert/queryReqConvertApproveList.asp?call='+tableCall+'&SID='+SID+'&type=2'+'&'+param});
		
	});
	enterEventRegister(getCurrentPageObj().attr("class"), function(){getCurrentPageObj().find("#view_query").click();});
	//重置
	var reset = currTab.find("#view_reset");
	...
	table.bootstrapTable({
		//请求后台的URL（*）
		url : dev_construction+'ReqConvert/queryReqConvertApproveList.asp?call='+tableCall+'&SID='+SID+'&type=2',
		method : 'get', //请求方式（*）   
	...
	//查看
	var view = currTab.find("#readAppConvert");
