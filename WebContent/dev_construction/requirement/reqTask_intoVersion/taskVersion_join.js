...
function initReqTaskVersionJoinBtn(){
	//提交并保存
	getCurrentPageObj().find('#taskVersion_join').click(function(){
		if(!vlidate(getCurrentPageObj().find("#g_reqtask_joinVersion"),"",true)){
			return ;
		}
		    var params={};
		    var req_task_id=getCurrentPageObj().find('#TVreq_task_id').val();
		    var version_id=getCurrentPageObj().find("#TVversion_id").val();
		    var version_content=getCurrentPageObj().find('#TVversion_content').val();
		    var version_name = getCurrentPageObj().find('#TVversion_name').val();
		    ...
		    params["version_type"] = getCurrentPageObj().find("#TVversion_type").val();
		    params["version_id"]=version_id;
		    params["version_content"]=version_content;
		    params["version_name"] = version_name;
		    
    		/******插入提醒参数*******/
    		params["b_code"] = getCurrentPageObj().find('#TVreq_task_code').val();
    		params["b_id"] = req_task_id;
    		params["remind_type"] = "PUB2017151";
    		params["b_name"] = getCurrentPageObj().find("#TAreq_task_name").val()+"（编号："+params["b_code"]+
    		"）已入版到"+getCurrentPageObj().find("#TVversion_name").val();
    		baseAjaxJsonp(dev_construction+"reqtask_intoVersion/updateReqTaskVersion.asp?SID="+SID+"&req_task_id="+req_task_id, params , function(data) {
...
//弹出版本pop
getCurrentPageObj().find('#TVversion_name').click(function(){	
	openTaskVersionPop("getTaskVersion",{versionsname:getCurrentPageObj().find('#TVversion_name'),versionsid:getCurrentPageObj().find('#TVversion_id'),versionstype:getCurrentPageObj().find("#TVversion_type"),vm:"01"});
});
//查看版本信息
getCurrentPageObj().find('#versionDetail_view').click(function(){
	var versions_id = getCurrentPageObj().find('#TVversion_id').val();
...
function checkVersion(req_id,version_id){
	baseAjaxJsonp(dev_construction+"reqtask_intoVersion/checkVersion.asp?SID="+SID+"&version_id="+version_id+"&req_id="+req_id, null , function(data) {
		  if (data != undefined && data != null && data.result=="true") {
			  getCurrentPageObj().find('#TVversion_name').val("");
			  getCurrentPageObj().find('#TVversion_id').val("");
			  getCurrentPageObj().find("#TVversion_type").val("");
			  alert("该立项项目已存在相同版本！");
		  }
		});
}

function viewReqDetailTV(){
  var req_id=getCurrentPageObj().find("#TVreq_id").val();
...
function initSubReqTaskList4Version(){
	var sub_req_id=getCurrentPageObj().find('#TVsub_req_id').val();
	var req_task_id=getCurrentPageObj().find('#TVreq_task_id').val();
	...
	$('#gReqTaskTableList4Version').bootstrapTable("destroy").bootstrapTable({
