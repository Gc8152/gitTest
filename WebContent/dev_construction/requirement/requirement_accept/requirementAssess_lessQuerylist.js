...
function initReqAssessListLayout(req_state) {
  ...
  initSelect(currTab.find("#req_state_reqDisQuery"),{value:"ITEM_CODE",text:"ITEM_NAME"},{dic_code:'G_DIC_REQUIREMENT_STATE'},(req_state==""?undefined:req_state),null);
 //初始化按钮  
 ...
 function initReqAssessListBtn(){
  ...
  currTab.find("#reqAssessQuery_put_org_code").click(function(){
			...
			openSelectTreeDiv($(this),"reqAssessQuery2_tree_id","SOrg/queryorgtreelist.asp",{"margin-top": "2px",width:"185px"},function(node){
 ...
 function initReqAssessQueryTable() {
		...
		getCurrentPageObj().find('#gReqForAssessTable').bootstrapTable("destroy").bootstrapTable({
					url :dev_construction+"requirement_accept/queryRequirementInfoForAcceptList.asp?SID="+SID+"&call="+reqAssQueryListCall+"&menu=req_assess"+"&req_state="+req_state,
					method : 'get', // 请求方式（*）
