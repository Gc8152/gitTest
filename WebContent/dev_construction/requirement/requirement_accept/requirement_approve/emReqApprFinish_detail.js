...
function initEmReqFinishDetailLayout(ids){
...
//	    		 currTab.find("div[name='update_system_file_describe']").attr("validate","v.required");
		...
		//初始化附件列表
		initVlidate($("#femReqAppr_analysisForm"));
		var tablefile = getCurrentPageObj().find("#femReqAppr_filetable");
		
		getSvnFileList(tablefile, getCurrentPageObj().find("#femreqAppr_fileview_modal"), data.REQ_CODE, "0101");
	},emreqdetailCall);
}
function initEmcontentPop(){
	...
	getCurrentPageObj().find('#apphistoryPop').append(
		...
		'<div class="ecitic-new">'+
			'<table id="fEmAFApprovalTableInfo" class="table table-bordered table-hover table-text-show"></table>'+
		'</div>'		
	);
...
function initEmReqApprovalDetailInfo(instance_id) {
	...
	getCurrentPageObj().find('#fEmAFApprovalTableInfo').bootstrapTable({
		...
		onLoadSuccess:function (){
			setRowspan("fEmAFApprovalTableInfo");
			gaveInfo();
		},
		columns : [{
			field : 'N_ID',
