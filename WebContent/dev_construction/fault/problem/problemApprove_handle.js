;function initProblemApproveHandleLayout(PROBLEM_ID){
	var currTab = getCurrentPageObj();
	var form = currTab.find("#problem_basic_table_approve");
	var form_solutionform = currTab.find("#problem_plan_solution");
	var table_task_add = currTab.find("#problem_task_addform");
	var REQ_TASK_ID=null;//全局变量  需求任务ID（判断新增还是修改）
	
	//返回
	currTab.find("#back_problemApproveInfo").click(function(){
		closeCurrPageTab();
	});
	
	//审批按钮
	currTab.find("#agree_approve").bind('click', function(e) {
		$("#mymodal_approveinfo").modal('show');
		currTab.find("#PROBLEM_APPROVE_SPAN").html("同意");
	});
	currTab.find("#disagree_approve").bind('click', function(e) {
		$("#mymodal_approveinfo").modal('show');
		currTab.find("#PROBLEM_APPROVE_SPAN").html("打回");
	});
	
	
	
	//审批提交操作
	var save_sumit = currTab.find("#approve_sumit");
	save_sumit.click(function(e){
	    var STATUS= currTab.find("#PROBLEM_APPROVE_SPAN").html()
	    ...
	    baseAjaxJsonp(dev_construction+"Problem/problemApprove.asp?call="+call+"&SID="+SID+"&PROBLEM_ID="+PROBLEM_ID+"&STATUS="+STATUS, null, function(data){
			if (data != undefined && data != null) {
				$("#mymodal_approveinfo").modal('hide');
				alert(data.msg);
				if(data.result=="true"){
					closeCurrPageTab();
				}
...
function TaskQueryList(PROBLEM_ID){
	...
	var currTab = getCurrentPageObj();
	var call = getMillisecond();
	var table = currTab.find("#problemapprove_handle_table");
