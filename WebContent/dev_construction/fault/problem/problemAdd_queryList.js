...
function initProblemListLayout(){
	...
	problem_Info.click(function(){
		...
		openInnerPageTab("problem_quenyInfo","问题单查看页面","dev_construction/fault/problem/problemAdd_quenyInfo.html",function(){
	...
	problem_add.click(function(){
		openInnerPageTab("problem_add1","问题单新增","dev_construction/fault/problem/problem_add.html",null);
	});
	//修改
	var problem_modify = currTab.find("#problem_modify");
	problem_modify.click(function(){
		var id = $("#problem_InfoTable").bootstrapTable('getSelections');
		if(id.length==1){
			var ids=$.map(id, function (row) {return row.PROBLEM_ID;});
			openInnerPageTab("problem_modify","问题单修改","dev_construction/fault/problem/problem_modify.html",null);
			initProblemUpdateBtn(ids);
		}else{
			alert("请选择一条数据进行删除");
			return;
		}
	...
	//删除
	currTab.find('#problem_delete').click(function(){
		var id = $("#problem_InfoTable").bootstrapTable('getSelections');
		if(id.length==1){
		var ids=$.map(id, function (row) {return row.PROBLEM_ID;});
		nconfirm("是否确定删除？",function(){
		baseAjaxJsonp(dev_construction+"Problem/problemDelete.asp?SID="+SID+"&PROBLEM_ID="+ids, null , function(data) {
		   if(data!=null&&data!=""&&data.result=="true"){
			   alert("删除成功!");
			   currTab.find('#problem_InfoTable').bootstrapTable('refresh');
		    }else{
			   alert("删除失败！");
		    }
		   });
	...
	//initTable();
		...
		//var param = form.serialize();
		/*var SERNO= currTab.find("#SERNO_ID").val();
		var INPUT_ID= currTab.find("#INPUT").val();*/
		var kind="1";//查询分类（用于区分列表查询还是审批查询）， 由于列表与审批查询字段不同要求
		
		table.bootstrapTable({
			//请求后台的URL（*）
			url : dev_construction+'Problem/problemQueryList.asp?call='+call+'&SID='+SID+'&kind='+kind,
			method : 'get', //请求方式（*）   
