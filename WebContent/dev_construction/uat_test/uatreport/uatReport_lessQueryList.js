...
var create_person = $("#currentLoginNo").val();
/**
 * 组装查询url 
 * @returns {String}
 */
function uatReportQueryUrl(){
	var url = dev_construction+'UatReport/queryLessReqSubReport.asp?call='+uatReport_queryList_call+'&SID='+SID+'&create_person='+create_person;
	var finds = getCurrentPageObj().find("#reqSubReportTerm [name]");
	for(var i=0; i<finds.length; i++){
		var obj=$(finds[i]);
		if($.trim(obj.val())!=""){
...
(function() {
	...
	getCurrentPageObj().find("#reqSubReportTable").bootstrapTable(
			{
				url : dev_construction+'UatReport/queryLessReqSubReport.asp?call='+uatReport_queryList_call+'&SID='+SID+'&create_person='+create_person,
				method : 'get', //请求方式（*）   
				...
				singleSelect: true,
				jsonpCallback: uatReport_queryList_call,
...
(function() {	
	...
	getCurrentPageObj().find("#upload").click(function(){
		...
		if(report_states[0] == '01') {//草拟
			alert("该需求点已有过上传操作，请修改！");
			return;
		}
