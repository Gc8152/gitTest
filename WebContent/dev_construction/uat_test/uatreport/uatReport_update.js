//初始化字典项
function initUatReportUploadInfoDic(accept_result, putin_result){
...
var uatReportUpdateUUID;
//初始化UAT报告上传信息
function initUatReportUploadInfo(p) {
	var call = getMillisecond()+'1';
	var url = dev_construction+'UatReport/queryOneUatReportUpdateInfo.asp?call='+call+'&SID='+SID+'&sub_req_id='+p;
	baseAjaxJsonp(url, null, function(data){
		if (data != undefined&&data!=null&&data.result=="true") {
			//详细信息
			if(data.report) {
				for(var k in data.report){
					getCurrentPageObj().find("[name^='A."+k+"']").val(data.report[k]);
				}
				uatReportUpdateUUID=data.report["uuid"];
				//计算缺陷数
				getCurrentPageObj().find("[name='aleave']").val(data.report["adefect_num"]-data.report["adefect_passnum"]);
				getCurrentPageObj().find("[name='bleave']").val(data.report["bdefect_num"]-data.report["bdefect_passnum"]);
				getCurrentPageObj().find("[name='cleave']").val(data.report["cdefect_num"]-data.report["cdefect_passnum"]);
				getCurrentPageObj().find("[name='dleave']").val(data.report["ddefect_num"]-data.report["ddefect_passnum"]);
				getCurrentPageObj().find("[name='allpass']").val(parseInt(data.report["adefect_passnum"])
						+parseInt(data.report["bdefect_passnum"])+parseInt(data.report["cdefect_passnum"])
						+parseInt(data.report["ddefect_passnum"]));
				//初始化字典项
				initUatReportUploadInfoDic(data.report["accept_result"], data.report["putin_result"]);				
			}
		}
	}, call);
}
...
(function(){
	...
	getCurrentPageObj().find("#saveUatReprot").click(function() {
		...
		//保存，这里应该判断附件中有无数据，有则文档已经上传，无则文件没有上传
		...
		var call = getMillisecond()+'1';
		var url = dev_construction+'UatReport/updateOrSubmitUatReportInfo.asp?call='+call+'&SID='+SID+'&flag=0';
	...
	getCurrentPageObj().find("#submitUatReprot").click(function() {
		...
		nconfirm("确定提交UAT测试报告吗？",function(){
			...
			items["biz_id"] = uatReportUpdateUUID.toUpperCase();//业务id
			approvalProcess(items,function(data){
				submitToApp(param);
			});
		});
	});
	function submitToApp(param){
		var param2 = {};
		param2["b_code"] = getCurrentPageObj().find("#sub_req_code").text();
		param2["b_id"] = getCurrentPageObj().find("#sub_req_id").val();
		param2["sub_req_id"] = getCurrentPageObj().find("#sub_req_id").val();
		param2["b_name"] = getCurrentPageObj().find("#sub_req_name").text()+"（编号："+param2["b_code"]+"）UAT报告已上传";;
		var call = getMillisecond()+'2';
		var url = dev_construction+'UatReport/updateOrSubmitUatReportInfo.asp?call='+call+'&SID='+SID+'&flag=1';
//		nconfirm("确定要提交测试报告吗？提交后将不能再次上传测试报告！想要再次上传报告请点击保存按钮即可",function(){
		baseAjaxJsonp(url, param, function(data){
			if (data != undefined&&data!=null&&data.result=="true") {
				alert("提交成功!",function(){
