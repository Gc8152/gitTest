//初始化字典项
(function(){
	var carr = ['00','01','03','04'];
	initSelect(getCurrentPageObj().find("[name='app_status']"),{value:"ITEM_CODE",text:"ITEM_NAME"},{dic_code:"I_DIC_INTER_ORDER_APP_STATUS"},"","",carr);
})();
...
function postUseInterAcceptTest(){
	...
	//接口申请查看
	$page.find('#interfaceCheck').click(function(){
		var selects = getCurrentPageObj().find("#ChangeAcceptingInfoTable").bootstrapTable("getSelections");
		if(selects.length == 0) {
			alert("请选择一条数据进行操作！");
			return;
		}
		closeAndOpenInnerPageTab("changeInterAccepted_info","接口受理查看","dev_application/changeInterAccept/changeInterAccept_queryInfo.html",function(){
			initTableInfo(selects[0]);
		});
	});
	
	//查询按钮
	var form = $page.find("#changeInterAcceptForm");//表单对象
	$page.find("#queryChangeAccepting").click(function() {		
		 var param = form.serialize();
		 $page.find('#ChangeAcceptingInfoTable').bootstrapTable('refresh',{
