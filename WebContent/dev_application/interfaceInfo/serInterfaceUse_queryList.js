var url = '';
//按钮方法
function initQueryInterUseButtonEventByUser(){
	var $page = getCurrentPageObj();
	$page.find("#interuseQuery").click(
			function() {
                var param=$page.find("#serInterfaceUse_queryList_form").serialize();
                $page.find('#serInterfaceTableInfo').bootstrapTable('refresh',{url:dev_application+'InterConUseQuery/queryconallinteruse.asp?SID='+SID+'&'+param});
	...
	$page.find("#interuseReset").click(function() {
		$page.find("#serInterfaceUse_queryList_form input").val("");
		var uselects = $page.find("#serInterfaceUse_queryList_form select");
		uselects.val(" ");
		uselects.select2();
	});
...
//查询列表显示table
function initInterUseTableInfoByUser() {
	 var param1=getCurrentPageObj().find("#serInterfaceUse_queryList_form").serialize();
	...
	getCurrentPageObj().find("#serInterfaceTableInfo").bootstrapTable(
			{
				//请求后台的URL（*）
				url:dev_application+'InterConUseQuery/queryconallinteruse.asp?SID='+SID+'&'+param1,
				method : 'get', //请求方式（*）   
				...
				columns : [ {
					...
					formatter : function(value, row, index) {
						...
						if(row.INTER_STATUS=="02"){//新建接口
								...
								"onclick='serInfcUse_InterDetail(\"" + index + "\")'>"+value+"</span>";
							}
						else{
	...
	function initFileUploadAction(){
		var $details = getCurrentPageObj().find("#serInterfaceTableInfo").find("a[name=inter_use_file_detail]");
		var $downloads = getCurrentPageObj().find("#serInterfaceTableInfo").find("a[name=inter_use_file_download]");
	...
	function getInterFileList(){
		var affectTableData = getCurrentPageObj().find("#serInterfaceTableInfo").bootstrapTable('getData');
		...
		if(businessArr.length>0){
			...
			if(fileData.length>0){
				for(var k in fileData){
					var file = fileData[k];
					var $div = getCurrentPageObj().find("#serInterfaceTableInfo").find("div[bid="+file["BUSINESS_CODE"]+"]");
...
//下拉框方法
function initInterUseQueryTypeByUser(){
	//初始化数据
...
function serInfcUse_InterDetail(index){
	var selects=getCurrentPageObj().find("#serInterfaceTableInfo").bootstrapTable('getData');
...
initInterUseQueryTypeByUser();
initQueryInterUseButtonEventByUser();
initInterUseTableInfoByUser();
