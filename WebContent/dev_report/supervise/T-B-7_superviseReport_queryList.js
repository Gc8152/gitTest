...
function SuperviseReportQueryUrl(){
	var url = dev_report+'SuperviseReport/queryAllSuperviseReport.asp?call='+SuperviseReport_queryList_call+'&SID='+SID+'&supervise_type=T-B-7';
	var finds = getCurrentPageObj().find("#superviseTerm [name]");
	for(var i=0; i<finds.length; i++){
		var obj=$(finds[i]);
		if($.trim(obj.val())!=""){
...
(function() {
	...
	getCurrentPageObj().find("#superviseReportTable").bootstrapTable(
			{
				url : dev_report+'SuperviseReport/queryAllSuperviseReport.asp?call='+SuperviseReport_queryList_call+'&SID='+SID+'&supervise_type=T-B-7',
				method : 'get', //请求方式（*）   
...
(function() {	
	...
	getCurrentPageObj().find("#add").click(function(){
		closeAndOpenInnerPageTab("superviseReport_add","新增","dev_report/supervise/T-B-7_superviseReport_add.html",function(){
...
function updateSuperviseReport(index) {
	closeAndOpenInnerPageTab("superviseReport_update","修改","dev_report/supervise/T-B-7_superviseReport_add.html",function(){
...
function detailSuperviseReport(index) {
	closeAndOpenInnerPageTab("superviseReport_detail","查看","dev_report/supervise/T-B-7_superviseReport_add.html",function(){
