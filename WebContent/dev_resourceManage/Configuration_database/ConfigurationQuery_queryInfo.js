...
function initTableInfo(item,statu) {
	var CONFIG_ID=item;
	var currTab = getCurrentPageObj();
	var url="";
	if(statu=="01"){
		 url=dev_resource+'ConfigApply/queryConfigInfo.asp?call='+changeCalls+'&SID='+SID+'&CONFIG_ID='+CONFIG_ID;
	}else{
		 url=dev_resource+'ConfigApply/queryConfigInfoAll.asp?call='+changeCalls+'&SID='+SID+'&CONFIG_ID='+CONFIG_ID;
	}
	...
	//debugger;
	getCurrentPageObj().find('#user_tableQuery').bootstrapTable({
	...
	baseAjaxJsonp(url,null,function(data){
		...
		baseAjaxJsonp(dev_resource+"ConfigApply/queryListSystemInfo.asp?SID="+SID+'&call='+changeCall+"&dev_flag="+dev_flag +"&PROJECT_MAN_ID="+project_man_id+"&SKILL_MAN_ID="+skill_man_id,null, function(data) {
			if(data.rows1.length!=0){
				$('#user_tableQuery').bootstrapTable("prepend",data.rows1[0]);
			}
			if(data.rows2.length!=0){
				$('#user_tableQuery').bootstrapTable("prepend",data.rows2[0]);
			}
		},changeCall);
	},changeCall);
