...
function initTableInfo(item) {
	...
	var url=dev_resource+'ConfigApply/queryConfigInfoAll.asp?call='+changeCalls+'&SID='+SID+'&CONFIG_ID='+CONFIG_ID;
	...
	currTab.find("#next_stepConfirm").click(function(){
		$('#myTabC a:last').tab('show');
	});
	...
	//debugger;
	getCurrentPageObj().find('#user_tableConfirmAdd').bootstrapTable({
	...
	$('#save_button').click(function(){
		...
		var url = dev_resource+'ConfigApply/updateConfigConfirm.asp?call='+changeCall1+'&SID='+SID+'&CONFIG_ID='+CONFIG_ID;
	...
	baseAjaxJsonp(url,null,function(data){
		...
		baseAjaxJsonp(dev_resource+"ConfigApply/queryListSystemInfo.asp?SID="+SID+'&call='+changeCall+"&dev_flag="+dev_flag +"&PROJECT_MAN_ID="+project_man_id+"&SKILL_MAN_ID="+skill_man_id,null, function(data) {
			if(data.rows1.length!=0){
				$('#user_tableConfirmAdd').bootstrapTable("prepend",data.rows1[0]);
			}
			if(data.rows2.length!=0){
				$('#user_tableConfirmAdd').bootstrapTable("prepend",data.rows2[0]);
			}
		},changeCall);
	},changeCall);
