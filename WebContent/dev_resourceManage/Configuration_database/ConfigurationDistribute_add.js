...
function initTableInfo(item) {
	...
	currTab.find("#next_stepDistribute").click(function(){
		$('#myTabD a:last').tab('show');
	});
	...
	//debugger;
	getCurrentPageObj().find('#user_tableDistributeAdd').bootstrapTable({
	...
	$('#save_button').click(function(){
		...
		if(!vlidate(getCurrentPageObj().find("#gDistributeInfo_add"),"",true)){
			return ;
		}
		
		var url = dev_resource+'ConfigApply/addConfigDistribute.asp?call='+changeCall1+'&SID='+SID+'&CONFIG_ID='+CONFIG_ID;
	...
	baseAjaxJsonp(url,null,function(data){
		...
		baseAjaxJsonp(dev_resource+"ConfigApply/queryListSystemInfo.asp?SID="+SID+'&call='+changeCall+"&dev_flag="+dev_flag +"&PROJECT_MAN_ID="+project_man_id+"&SKILL_MAN_ID="+skill_man_id,null, function(data) {
			if(data.rows1.length!=0){
				$('#user_tableDistributeAdd').bootstrapTable("prepend",data.rows1[0]);
			}
			if(data.rows2.length!=0){
				$('#user_tableDistributeAdd').bootstrapTable("prepend",data.rows2[0]);
			}
		},changeCall);
	},changeCall);
