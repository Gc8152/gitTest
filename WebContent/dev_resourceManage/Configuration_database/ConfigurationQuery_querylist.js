...
function initConfigAccept(){
	...
	getCurrentPageObj().find("#reset").click(function() {
		getCurrentPageObj().find("#configAcceptFormAll input").val("");
		getCurrentPageObj().find("#configAcceptFormAll select").val(" ").select2();
	});
	...
	getCurrentPageObj().find('#configInfo').click(function(){
		...
		closePageTab("config_info");
		openInnerPageTab("config_info","查看配置信息","dev_resourceManage/Configuration_database/ConfigurationQuery_queryInfo.html",function(){
			initTableInfo(selects[0].CONFIG_ID,selects[0].STATU);
		});
	});
...
function getConfigUrl(){
	...
	var url = dev_resource+'ConfigApply/queryAllConfigInfo.asp?call='+queryList_call+'&SID='+SID+"&req_finish_time="+req_finish_time+"&req_finish_time1="+req_finish_time1;
	var param = getCurrentPageObj().find("#configAcceptFormAll").serialize();
	return url+"&"+param;
}
