...
function initConfigAccept(){
	...
	getCurrentPageObj().find("#reset").click(function() {
		getCurrentPageObj().find("#configAcceptFormDistribute input").val("");
		getCurrentPageObj().find("#configAcceptFormDistribute select").val(" ").select2();
	});
	...
	currTab.find('#configAccept').click(function(){
		...
		closePageTab("config_distribute1");
		openInnerPageTab("config_distribute1","配置库分配","dev_resourceManage/Configuration_database/ConfigurationDistribute_add.html",function(){
	...
	getCurrentPageObj().find('#configInfo').click(function(){
		...
		closeAndOpenInnerPageTab("config_info","查看配置信息","dev_resourceManage/Configuration_database/ConfigurationDistribute_queryInfo.html",function(){
			initTableInfo(selects[0].CONFIG_ID);
		});
	});
...
function getConfigUrl(){
	var req_finish_time=getCurrentPageObj().find("#req_finish_time_useInterApp").val();
	var req_finish_time1=getCurrentPageObj().find("#req_finish_time1_useInterApp").val();
	
	var url = dev_resource+'ConfigApply/queryConfigDistribute.asp?call='+queryList_call+'&SID='+SID+"&req_finish_time="+req_finish_time+"&req_finish_time1="+req_finish_time1;
	var param = getCurrentPageObj().find("#configAcceptFormDistribute").serialize();
	return url+"&"+param;
}
