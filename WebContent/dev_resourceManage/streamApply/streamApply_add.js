initSelect(getCurrentPageObj().find("#stream_type"),{value:"ITEM_CODE",text:"ITEM_NAME"},{dic_code:"S_DIC_STREAM_TYPE"},'01',null);
//自动赋值
...
function initBasicInfo(item){
	for ( var k in item) {
		...
		if(k=="apply_desc"){
			...
			initSelect(getCurrentPageObj().find("#stream_type"),{value:"ITEM_CODE",text:"ITEM_NAME"},{dic_code:"S_DIC_YN"},str);
			if(str=="00"){
...
/*function initStreamTask(system_id){
	...
	getCurrentPageObj().find('#requirement_task').bootstrapTable("destroy").bootstrapTable({
				url :dev_resource+"StreamApply/findTaskByStream.asp?SID="+SID+"&system_id="+system_id+"&call="+reqTaskListCall,
				method : 'get', // 请求方式（*）
