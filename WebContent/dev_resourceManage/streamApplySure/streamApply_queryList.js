...
function initClickButtonEvent(){
	...
	getCurrentPageObj().find("#queryStreamApply").click(function(){
		...
		getCurrentPageObj().find("#StreamApplySureTableInfo").bootstrapTable('refresh',
				{url:dev_resource+'StreamApply/queryStreamApplySure.asp?stream_app_status=04&call='+quaryStreamCall+'&SID='+SID+'&'+params});		
	});
	//enter触发查询
	...
	//流信息确认
	getCurrentPageObj().find("#streamApply_sure").unbind("click");
	getCurrentPageObj().find("#streamApply_sure").click(function(){
		var selections = getCurrentPageObj().find("#StreamApplySureTableInfo").bootstrapTable('getSelections');
		if(selections.length != 1) {
			alert("请选择一条数据进行操作!");
			return;
		}
		if(selections[0].STREAM_APP_STATUS=="04"){
			closeAndOpenInnerPageTab("stream_sure","流信息确认","dev_resourceManage/streamApplySure/streamApply_sure.html",function(){
				initStreamInfoForSure(selections[0]);
			...
			alert("请选择一条待确认数据进行操作!");
			return;
		}
	});	
	
	//批量确认
	getCurrentPageObj().find("#bantchStreamApply_sure").unbind("click");
	getCurrentPageObj().find("#bantchStreamApply_sure").click(function(){
		var selections = getCurrentPageObj().find("#StreamApplySureTableInfo").bootstrapTable('getSelections');
		if(selections.length > 0) {
			var sendData = getCurrentPageObj().find("#StreamApplySureTableInfo").bootstrapTable('getSelections');
			var applyIds = $.map(sendData,function(row){
				return row.STREAM_APP_ID;
			}).join(",");
			var bantchCall = getMillisecond();
			baseAjaxJsonp(dev_resource+'StreamApply/bantchStreamApplySure.asp?call='+bantchCall+'&SID='+SID+
					"&applyId="+applyIds, null , function(data) {
				if (data != undefined && data != null && data.result==true) {
					getCurrentPageObj().find("#StreamApplySureTableInfo").bootstrapTable('refresh',
							{url:dev_resource+'StreamApply/queryStreamApplySure.asp?stream_app_status=04&call='+quaryStreamCall+'&SID='+SID});
					alert("确认成功！");
				}else{
					alert("确认失败！"+data.msg);
				}
...
function initStreamAppDicType(){
	//初始化数据,属性类型
	initSelect(getCurrentPageObj().find("#stream_app_status"),{value:"ITEM_CODE",text:"ITEM_NAME"},{dic_code:"R_DIC_STREAM_APP_STATUS"},'04');
