...
//初始化版本Select
function initProjectVersSelect(project_id){
	
	getCurrentPageObj().find("#project_id").val(project_id);
	...
	baseAjaxJsonp(dev_planwork+'Wbs/queryProjectVersSelect.asp?project_id='+ project_id + "&SID=" + SID + "&call=" +SelectVersCall,null,function(data) {
  		if (data != undefined&&data!=null) {
  			...
  			if(r != null && r.length > 0) {
      			...
      			//每次加载清楚上次统计的数据（放在循环外）
      			getCurrentPageObj().find("option[name='ProjectSelectVersList']").remove();
      			for(var i = 0;i<VersArr.length; i++) {
      				appendselectVersOption("selectVers",VersArr[i]);
      			}
      			//初始化里程碑      			
				queryMilestoneList(project_id,VersArr[0].value);
      		}  			  			  			
...
function appendselectVersOption(Select,vers){
  	...
  	var option = "<option name='ProjectSelectVersList' value="+vers.value+">"+vers.html+"</option>";
  	Obj.append(option);
...
function queryMilestoneList(project_id,version_id){
