...
//		alert(ids.toString());
		...
		baseAjaxJsonp(dev_planwork + 'workCon/approval.asp?call=' + call+ '&SID=' + SID, {ids:ids.toString(),status:'02'},//02审批通过操作
			...
			if(msg.result){
				alert("已经审批通过！");
                inittaskDetailsInfoPage(inittaskDetailsInfoPageparam);
			 }
			}, call);
	});
...
function inittaskDetailsInfoPage(callparams){
    inittaskDetailsInfoPageparam=callparams;
	var url = dev_planwork + 'workCon/queryPersonalTaskDetails02.asp?call=' + mcall+ '&SID=' + SID;
