...
/*************    方法     **************/
    ...
    attend[1]="<a class='data_normal'><div class='data_txt'>上班</div></a>"//工作日未报工
    attend[2]="<a class='data_normal'><div class='data_txt2'>上班</div></a>"//工作日已报工
			...
			if(i < dateFirstDay && dateMonth>=1){//非一月上月日期
				...
				if($.inArray(b,dayStr2)>=0){
					if($.inArray(b,bookDay2)>=0){
						dateObj.eq(i).after(attend[2]).after("<a style='display:none'>"+selectyear.val()+"年"+(parseInt(selectmonth.val())+1)+"月"+(i+1-dateFirstDay)+"日"+"</a>");
					}else{
