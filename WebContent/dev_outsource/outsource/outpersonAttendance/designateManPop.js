function proManPop1(obj,callparams){
	$("#defect_designate_modal").remove();
	//加载pop框内容
	getCurrentPageObj().find("#"+obj).load("dev_outsource/outsource/outpersonAttendance/designateManPop.html",{},function(){
		...
		modObj.find("[tb='projectManPopTable']").bootstrapTable("destroy").bootstrapTable({
					url : dev_outsource+"optattendance/queryProjectMan.asp?SID="+SID+"&call="+manCall,
					method : 'get', // 请求方式（*）
					...
					onDblClickRow:function(row){
						callparams.op_name.val(row.USER_NAME);
						callparams.user_no.val(row.USER_NO);
						callparams.op_code.val(row.OUTPERSION_ID);
						modObj.modal("hide");
					},
					columns : [ {
						field : 'USER_NO',
		...
		modObj.find("#manSearch").click(function(){
			...
			modObj.find("[tb='projectManPopTable']").bootstrapTable('refresh',{url:dev_outsource+"optattendance/queryProjectMan.asp?SID="+SID+"&call="+manCall+"&"+param,});
		});
