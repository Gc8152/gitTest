...
function initOutPersonFindLayout(){
	...
	function queryOutPersonUrl(){
		var url= dev_outsource + "outperson/findOutPersonInfo.asp?a=1&SID="+SID+"&call="+call_in+'&query_type=enter_approve';
		var sts="";
	...
	function initClickEvents(){
		...
		initSelect(getCurrentPageObj().find("#op_types"),{value:"ITEM_CODE",text:"ITEM_NAME"},{dic_code:"C_DIC_PURCH_TYPE"});	//人员采购类型
		...
		//审批
		getCurrentPageObj().find("#outperson_approve1").unbind("click");//审批
		getCurrentPageObj().find("#outperson_approve1").click(function(){
			...
			closeAndOpenInnerPageTab("outpersonInbankDetailOne","外包人员入场审批","dev_outsource/outsource/outpersonEntrance/outperson_approve.html",function(){
		...
		//查询按钮
		getCurrentPageObj().find("#queryOutPersonInfo1").unbind("click");
		getCurrentPageObj().find("#queryOutPersonInfo1").click(function(){
			getCurrentPageObj().find("#outpersonTableInfo").bootstrapTable("refresh",{url:queryOutPersonUrl()});
		});
		//enter触发查询
		enterEventRegister(getCurrentPageObj().attr("class"), function(){getCurrentPageObj().find("#queryOutPersonInfo1").click();});
		//重置按钮
		getCurrentPageObj().find("#resetOutPersonInfo1").unbind("click");//重置按钮
		getCurrentPageObj().find("#resetOutPersonInfo1").click(function(){
			getCurrentPageObj().find("input[name^='FD.']").val("");
			getCurrentPageObj().find("#FDsupplier_name").val("点击选择供应商");
			getCurrentPageObj().find("#FD_op_office").val("点击选择行内归属部门");
			getCurrentPageObj().find("#staffInfoList select").val(" ").select2();
		});
		getCurrentPageObj().find("#outperson_Import1").unbind("click");//导入按钮
		getCurrentPageObj().find("#outperson_Import1").click(function(){
		...
		obj=getCurrentPageObj().find("#outperson_export1");//外包人员导出
		obj.unbind("click");
		obj.click(function(){
...
//		}
	...
	function initTable() {
		...
		getCurrentPageObj().find("#outpersonTableInfo").bootstrapTable({
					url : dev_outsource + 'outperson/findOutPersonInfo.asp?SID='+SID+"&call="+call_in+'&query_type=enter_approve',
					method : 'get', //请求方式（*）   
