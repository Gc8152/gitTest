 ...
 * @returns {String}
 */
var call_ranks = getMillisecond();
function Urls(){
	var url=dev_outsource+'outperson/queryOutPersonRankInfo.asp?call='+call_ranks+'&SID='+SID+'&query=queryList+&spstate_type=01';
...
(function(){
	...
	getCurrentPageObj().find("#outpersonRank_Del").click(function(){
		var dt=getCurrentPageObj().find("#outpersonTableInfoRanks").bootstrapTable('getSelections');
		if(dt.length!=1){
			alert("请选择一条数据!");
			return;
		}
		...
		nconfirm("确定需要删除该数据？",function(){
			...
			baseAjaxJsonp(dev_outsource+'outperson/deleteRankInfo.asp?call='+call+'&SID='+SID,{id:dt[0]["ID"]},function(data){	
				if(data&&data.result=="true"){
					getCurrentPageObj().find("#outpersonTableInfoRanks").bootstrapTable("refresh");
					alert("删除成功");
				}else{
					alert("删除失败");
				}
	...
	getCurrentPageObj().find("#outpersonRank_Print").click(function(){
		var ids = getCurrentPageObj().find("#outpersonTableInfoRanks").bootstrapTable('getSelections');
		if(ids.length!=1){
			alert("请选择一条数据进行打印!");
			return ;
		}
...
/*	//职级评定打印
	...
	//查询按钮
	getCurrentPageObj().find("#re_queryOutPersonInfoRank").unbind("click");
	getCurrentPageObj().find("#re_queryOutPersonInfoRank").click(function(){
		getCurrentPageObj().find("#outpersonTableInfoRanks").bootstrapTable("refresh",{url:Urls()});
	});
	//重置按钮
	getCurrentPageObj().find("#re_resetOutPersonInfoRank").unbind("click");//重置按钮
	getCurrentPageObj().find("#re_resetOutPersonInfoRank").click(function(){
...
//查询列表显示table
(function() {
	getCurrentPageObj().find("#outpersonTableInfoRanks").bootstrapTable({
				url : dev_outsource+'outperson/queryOutPersonRankInfo.asp?call='+call_ranks+'&SID='+SID+'&query=queryList&spstate_type=01',
				method : 'get', //请求方式（*）   
