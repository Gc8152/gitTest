 ...
 * @returns {String}
 */
function queryLeaveUrls(){
	var url = dev_outsource+'outperson/findOutPersonInfo.asp?call='+leavecall+'&SID='+SID+'&query=queryList&menu_no=leave';
	var sts="";
...
(function() {
	...
	getCurrentPageObj().find("#outperson_Edit").click(function(){
		var dt=getCurrentPageObj().find("#OutLeaveTableInfo").bootstrapTable('getSelections');
		if(dt.length!=1){
			alert("请选择一条数据!");
			return;
		}
		...
		nconfirm("确定需要删除该数据？",function(){
			...
			baseAjaxJsonp(dev_outsource+'outperson/deleteOutPerson.asp?call='+call_op+'&SID='+SID,{op_id:dt[0]["OP_ID"]},function(data){
				if(data&&data.result=="true"){
					getCurrentPageObj().find("#OutLeaveTableInfo").bootstrapTable("refresh");
					alert("删除成功");
				}else{
					alert("删除失败");
				}
...
/*	getCurrentPageObj().find("#outperson_Import").unbind("click");//导入按钮
	...
	//查询按钮
	getCurrentPageObj().find("#queryLeaveInfo").unbind("click");
	getCurrentPageObj().find("#queryLeaveInfo").click(function(){
		getCurrentPageObj().find("#OutLeaveTableInfo").bootstrapTable("refresh",{url:queryLeaveUrls()});
	});
	//重置按钮
	getCurrentPageObj().find("#resetLeaveInfo").unbind("click");//重置按钮
	getCurrentPageObj().find("#resetLeaveInfo").click(function(){
	...
	getCurrentPageObj().find("#OutLeaveTableInfo").bootstrapTable({
				url : queryLeaveUrls(),
				method : 'get', //请求方式（*）   
