 ...
 */
function queryUrls(){
	var url = dev_outsource+'outperson/findOutPersonInfo.asp?call='+getback+'&SID='+SID+'&query=queryList';
	var sts="";
...
(function() {
	...
	getCurrentPageObj().find("#outperson_Edit").click(function(){
		var dt=getCurrentPageObj().find("#OutTableInfo").bootstrapTable('getSelections');
		if(dt.length!=1){
			alert("请选择一条数据!");
			return;
		}
		...
		nconfirm("确定需要删除该数据？",function(){
			...
			baseAjaxJsonp(dev_outsource+'outperson/deleteOutPerson.asp?call='+call_op+'&SID='+SID,{op_id:dt[0]["OP_ID"]},function(data){
				if(data&&data.result=="true"){
					getCurrentPageObj().find("#OutTableInfo").bootstrapTable("refresh");
					alert("删除成功");
				}else{
					alert("删除失败");
				}
...
/*	getCurrentPageObj().find("#outperson_Import").unbind("click");//导入按钮
	...
	//查询按钮
	getCurrentPageObj().find("#queryInfo").unbind("click");
	getCurrentPageObj().find("#queryInfo").click(function(){
		getCurrentPageObj().find("#OutTableInfo").bootstrapTable("refresh",{url:queryUrls()});
	});
	//重置按钮
	getCurrentPageObj().find("#resetInfo").unbind("click");//重置按钮
	getCurrentPageObj().find("#resetInfo").click(function(){
	...
	getCurrentPageObj().find("#OutTableInfo").bootstrapTable({
				url : queryUrls(),
				method : 'get', //请求方式（*）   
