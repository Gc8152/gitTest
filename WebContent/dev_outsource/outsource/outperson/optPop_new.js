...
function openOptPop_new(id,callparams,callback){
	//先清除
	getCurrentPageObj().find('#myModal_opt_new').remove();
	//getCurrentPageObj().find('#'+id).empty();
	getCurrentPageObj().find("#"+id).load("dev_outsource/outsource/outperson/optPop_new.html",{},function(){
		getCurrentPageObj().find("#myModal_opt_new").modal("show");
		var url = dev_outsource+"outperson/findOutPersonInfo.asp?a=1";
		//callparams获取到所有外包人员的信息
		optPop_new("#pop_optTable",url,callparams);
	});
	if(callback){
...
function openOptPopBySUser(id,callparams,callback){
	...
	getCurrentPageObj().find("#"+id).load("dev_outsource/outsource/outperson/optPop_new.html",{},function(){
		getCurrentPageObj().find("#myModal_opt_new").modal("show");
		var url = dev_outsource+"outperson/findOutPersonPop.asp?a=1";
		//callparams获取到所有外包人员的信息
		optPop_new("#pop_optTable",url,callparams);
	});
	if(callback){
...
/**
	 * 用户POP框
	 */
function optPop_new(optTable,optUrl,optParam){
	...
	//查询所有行员POP框
	getCurrentPageObj().find("#myModal_opt_new").find(optTable).bootstrapTable("destroy").bootstrapTable({
				//请求后台的URL（*）
				url : optUrl+'&call=jq_1520840212928_new&SID='+SID,
				method : 'get', //请求方式（*）   
				...
				jsonpCallback:"jq_1520840212928_new",
				onDblClickRow:function(row){
					if(singleSelect){
						getCurrentPageObj().find('#myModal_opt_new').modal('hide');
						if(optParam.no){//外包人员编号
							optParam.no.val(row.OP_CODE);
						}
	...
	getCurrentPageObj().find("#pop_optSearch").click(function(){
		...
		getCurrentPageObj().find("#myModal_opt_new").find(optTable).bootstrapTable('refresh',{url:optUrl+"&SID="+SID+"&call=jq_1520840212928_new&op_name="+escape(encodeURIComponent(pop_optname))+"&op_code="+pop_optCode});//+"&supplier_id="+pop_opt_supplier_id+"&staff_id="+pop_opt_staff_id});
	});
}
