 ...
 * @returns {String}
 */
function queryOutPersonUrls(){
	var url = dev_outsource+'outperson/findOutPersonInfo.asp?call='+callTables+'&SID='+SID+'&query=queryList';
	var sts="";
	...
	var fds=getCurrentPageObj().find("input[name^='FB.']");//name为FD.entranceLeave的输入框表示如离场发起查询
...
(function() {
	...
	var obj=getCurrentPageObj().find("#FB_op_office");//实施处室
	obj.unbind("click");
	obj.click(function(){
		openSelectTreeDivToBody($(this),"op_office_pop_fd_tree","SOrg/queryorgtreelist.asp",30,function(node){
			getCurrentPageObj().find("#FB_op_office").val(node.name);
			getCurrentPageObj().find("input[name='FB.op_office']").val(node.id);
		});
	});
	...
	obj.click(function(){
		openSupplierPop("outperson_contractSupplier_Pop",
				{singleSelect:true,parent_company:getCurrentPageObj().find("#FDsupplier_name"),parent_sup_num:getCurrentPageObj().find("input[name='FB.supplier_id']")});
	});
	...
	getCurrentPageObj().find("#outperson_Edit").click(function(){
		var dt=getCurrentPageObj().find("#outpersonTable").bootstrapTable('getSelections');
		if(dt.length!=1){
			alert("请选择一条数据!");
			return;
		}
...
//		}
		...
		nconfirm("确定需要删除该数据？",function(){
			...
			baseAjaxJsonp(dev_outsource+'outperson/deleteOutPerson.asp?call='+call_op+'&SID='+SID,{op_id:dt[0]["OP_ID"]},function(data){
				if(data&&data.result=="true"){
					getCurrentPageObj().find("#outpersonTable").bootstrapTable("refresh");
					alert("删除成功");
				}else{
					alert("删除失败");
				}
	...
	//查看入场
	getCurrentPageObj().find("#outperson_Detaill").unbind("click");//查看详情
	getCurrentPageObj().find("#outperson_Detaill").click(function(){
		var dt=getCurrentPageObj().find("#outpersonTable").bootstrapTable('getSelections');
		//流程id
	...
	//入场申请
	getCurrentPageObj().find("#outperson_Apply").unbind("click");
	getCurrentPageObj().find("#outperson_Apply").click(function(){
		var dt=getCurrentPageObj().find("#outpersonTable").bootstrapTable('getSelections');
		if(!dt||dt.length==0){
			alert("请选择一条数据!");
			return;
...
/*	getCurrentPageObj().find("#outperson_Import").unbind("click");//导入按钮
	...
	obj.click(function(){
		...
		var op_office=getCurrentPageObj().find("#FB_op_office").val();//实施处室
	...
	//查询按钮
	getCurrentPageObj().find("#queryOutPerson").unbind("click");
	getCurrentPageObj().find("#queryOutPerson").click(function(){
		getCurrentPageObj().find("#outpersonTable").bootstrapTable("refresh",{url:queryOutPersonUrls()});
	});
	//重置按钮
	getCurrentPageObj().find("#resetOutPerson").unbind("click");//重置按钮
	getCurrentPageObj().find("#resetOutPerson").click(function(){
		getCurrentPageObj().find("input[name^='FB.']:visible").val(" ");
		getCurrentPageObj().find("#FDsupplier_name").val("");
		getCurrentPageObj().find("#staffInfoList select:visible").val(" ").select2();
		getCurrentPageObj().find("input[name='FB.supplier_id'],input[name='FB.op_office']").val(" ");
	});
	...
	getCurrentPageObj().find("#outpersonTable").bootstrapTable({
				url : queryOutPersonUrls(),
				method : 'get', //请求方式（*）   
