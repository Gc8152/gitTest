var k=1;
function initOutPersonRankDetail(op_id,flowid,spstate,flowurl,flag){
...
//					"<td ><div id='own_project"+k+"' name='own_project'  ></div></td>"+
					...
					"<td ><div type=\"text\"   name=\"op_protocol_level\" id=\"level_name"+k+"\"></div></td>"+
					//申请级别
					"<td ><div type=\"text\"   name=\"apply_grade\" id=\"Grade_name"+k+"\" ></div></td>"+
					//公司名称
					"<td ><div type=\"text\" name=\"op_company\" id=\"op_company"+k+"\" ></div></td>"+
					//评定日期
					...
					for (var p in rankPerson ){
						if(p == 'GRADE_NAME'){
							getCurrentPageObj().find("#Grade_name"+k).html(rankPerson[p]);
						}
						getCurrentPageObj().find("#"+p.toLowerCase()+k).html(rankPerson[p]);
					}
					//序号自增
	...
	//审批流程
//	initWorkFlowDetailInfo(flowid,"	",spstate,flowurl);//workflow_rank页面审批意见table的id
}