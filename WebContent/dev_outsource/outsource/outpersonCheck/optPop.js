...
function openOptForCheckPop(id,callparams,callback){
	//先清除
	getCurrentPageObj().find('#myModal_opt').remove();
	//getCurrentPageObj().find('#'+id).empty();
	getCurrentPageObj().find("#"+id).load("dev_outsource/outsource/outpersonCheck/optPop.html",{},function(){
		getCurrentPageObj().find("#myModal_opt").modal("show");
		var url = dev_outsource+"outperson/findOutPersonInfo.asp";
		//callparams获取到所有外包人员的信息
		optForCheckPop("#pop_optTable",url,callparams);
	});
	if(callback){
...
/**
	 * 用户POP框
	 */
function optForCheckPop(optTable,optUrl,optParam){
	...
	getCurrentPageObj().find(optTable).bootstrapTable("destroy").bootstrapTable({
				...
				onDblClickRow:function(row){
					if(singleSelect){
						if(optParam.func_call){
							optParam.func_call(row);
						}
						...
						getCurrentPageObj().find("#OCA_check_person").val($("#currentLoginName").val());
...
//			var pop_opt_staff_id =  $.trim(getCurrentPageObj().find("#pop_opt_staff_id").val());
		getCurrentPageObj().find(optTable).bootstrapTable('refresh',{url:optUrl+"?&op_name="+escape(encodeURIComponent(pop_optname))+"&op_code="+pop_optCode});//+"&supplier_id="+pop_opt_supplier_id+"&staff_id="+pop_opt_staff_id});
	});
}
