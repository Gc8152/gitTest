...
function getSystemPopUrl1() {
	...
	var url = dev_test+'addDefect/queryApplication.asp?SID='+SID;
 ...
 */
function openSystemPop1(id,callparams){
	getCurrentPageObj().find('#myModal_system').remove();	
	getCurrentPageObj().find("#"+id).load("dev_test/defectManagement/add/systemPop.html",{},function(){
				...
				//请求后台的URL（*）
					...
					onDblClickRow:function(row){	//双击行事件
							...
							if(systemParam.duty_name){
								...
								systemParam.duty_name.attr("disabled","disabled");
							}
