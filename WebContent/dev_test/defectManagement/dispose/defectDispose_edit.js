...
//	});
	...
	$page.find("[btn='fix_defect']").click(function(){
		...
		//$page.find("input[name='IU.DISPOSE_TIME']").val(getCurrentYMD());
	...
	//转交处理人
	$page.find("[btn='dispose_defect']").click(function(){
		$page.find("#dispose_person").modal('show');

	});
	
	//转交处理人保存
	$page.find("[btn='dispose_save']").click(function(){
		if(!vlidate($page,"",true)){
			return ;
		}
		var params = getPageParam("IU");
		params["TYPE"] = 'dispose';
		$page.find("#dispose_person").modal('hide');
		saveDispose(params);
	});
...
//	});
	...
	function initDefectInfo(item){
		...
		if(item.CHANGE_GRADE_REASON!=undefined){
			$page.find("tr[class='hidden']").show(); 
		}
