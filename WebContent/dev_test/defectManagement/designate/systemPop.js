 ...
 * @returns
 */
function getSystemPopUrl2(sys_id) {
	var condition = getCurrentPageObj().find("#pop_sys_condition [name]");
	var url = dev_test+'addDefect/queryApplication.asp?SID='+SID+'&discover=discover&sys_id='+sys_id;
 ...
 */
function openSystemPop2(id,callparams,sys_id){
	getCurrentPageObj().find('#myModal_system').remove();	
	getCurrentPageObj().find("#"+id).load("dev_test/defectManagement/designate/systemPop.html",{},function(){
		getCurrentPageObj().find("#myModal_system").modal("show");
		initSystemPop2("#pop_systemTable", getSystemPopUrl2(sys_id), callparams,sys_id);
 ...
 * 初始化应用模态框
 */
function initSystemPop2(systemTable,systemUrl,systemParam,sys_id){
	//分页
	...
	getCurrentPageObj().find(systemTable).bootstrapTable("destroy").bootstrapTable({
				//请求后台的URL（*）
					...
					onDblClickRow:function(row){	//双击行事件
							...
							if(systemParam.duty_id){
								systemParam.duty_id.val(row.PROJECT_MAN_ID);
							}
							if(systemParam.duty_name){
								systemParam.duty_name.val(row.USER_NAME);
							}
	...
	getCurrentPageObj().find("#pop_systemSearch").click(function(){
		getCurrentPageObj().find(systemTable).bootstrapTable('refresh',{url:getSystemPopUrl2(sys_id)});
	});
