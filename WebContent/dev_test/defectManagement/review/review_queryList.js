...
function initReviewList(){
	...
	var formObj = $page.find("#reviewForm");//表单对象
	var disEditCall = getMillisecond();//table回调方法名
	var reviewTable = $page.find("[tb='reviewTable']");
	
	//初始化列表
	initReviewTable();
	//重置按钮
	$page.find("[name='resetReview']").click(function(){
	...
	//查询按钮
	 $page.find("[name='queryReview']").click(function(){
		 var param = formObj.serialize();
		 reviewTable.bootstrapTable('refresh',{
				url:dev_test+"reviewDefect/queryReviewDefectList.asp?SID=" + SID + "&call=" + disEditCall +'&'+param});
	 });
	//enter触发查询
		enterEventRegister(getCurrentPageObj().attr("class"), function(){getCurrentPageObj().find("[name='queryReview']").click();});
	 
	//审核缺陷
	 $page.find("button[name='reviewDefect']").click(function(){
			var seles = reviewTable.bootstrapTable("getSelections");
			if(seles.length!=1){
					alert("请选择一条数据!");
					return;
			}

		 closeAndOpenInnerPageTab("reviewDefect","审核缺陷","dev_test/defectManagement/review/review_edit.html", function(){
			 reviewDefect(seles[0]);
			});
	 });
	 
	 
	//拒绝缺陷
	 $page.find("button[name='refuseDefect']").click(function(){
		 var seles = reviewTable.bootstrapTable("getSelections");
			if(seles.length!=1){
					alert("请选择一条数据进行查看!");
					return;
	...
	function saveReview(params){
		...
		baseAjaxJsonp(dev_test+"reviewDefect/saveReview.asp?SID=" + SID + "&call=" + sCall, params, function(data) {
			if(data && data.result=="true"){
				getCurrentPageObj().find("[tb='reviewTable']").bootstrapTable("refresh");
				//alert(data.msg);
			}else{
				alert(data.msg);
				getCurrentPageObj().find("[tb='reviewTable']").bootstrapTable("refresh");
	...
	function initReviewTable() {
		...
		reviewTable.bootstrapTable({
					url : dev_test+"reviewDefect/queryReviewDefectList.asp?SID=" + SID + "&call=" + disEditCall,
					method : 'get', // 请求方式（*）
