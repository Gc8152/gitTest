		...
		<div class="ecitic-inquire" id="ecitic-inquire">
			<div class="ecitic-table" id="ecitic-table">
				<table width="100%" border="0" cellspacing="0" cellpadding="0" class="table table-bordered table-bor-pad">
					<tr>
						<td width="11%" class="ecitic-right">子需求编号：</td>
						<td width="21.333%"><input type="text" name="P.sub_req_code" id="P_sub_req_code"/></td>
						<td width="11%" class="ecitic-right">任务编号：</td>
						<td width="21.333%"><input type="text" name="P.req_task_code" id="P_req_task_code"/></td>
...
$(document).ready(function(){	
	...
	$("#queryprdPermitDetail").click(function(){
		report.init(); //初始化报表控件		
		report.setFile("product_approve_detail.raq");//设置要显示的报表文件   
		report.setSaveAsName("投产准入明细表");
		report.putParam("org_no","<%=org_code%>");
		report.putParam("user_id","<%=suser.getUser_no() %>");
		report.putParam("sub_req_code",encodeURIComponent($.trim($("#P_sub_req_code").val())));
		report.putParam("req_task_code",encodeURIComponent($.trim($("#P_req_task_code").val())));
